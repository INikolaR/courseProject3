...
TEST(GivensLayer_backwardCalcGradient, assertion1) {
    ...
    Matrix diff_cumulative_u = u - Matrix{{15.6733}, {21.5255}};
    Matrix diff_cumulative_z = z - Matrix{{0.470871}, {0.168168}};
    EXPECT_TRUE(1e-5 > diff_g.norm());
    // EXPECT_TRUE(1e-5 > diff_cumulative_u.norm());
    // EXPECT_TRUE(1e-5 > diff_cumulative_z.norm());
}

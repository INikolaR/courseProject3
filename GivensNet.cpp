...
double GivensNet::loss(const std::vector<TrainUnit>& dataset,
...
double GivensNet::accuracy(const std::vector<TrainUnit> dataset) const {
    assert(!dataset.empty() && "dataset should not be empty");
...
void GivensNet::trainOneBatch(std::vector<TrainUnit>::const_iterator begin,
    ...
    std::vector<SVD> to_update = trainOneUnit(begin->x, begin->y, loss);
    for (auto it = begin + 1; it != end; ++it) {
        std::vector<SVD> add_to_update = trainOneUnit(it->x, it->y, loss);
...
std::vector<SVD> GivensNet::trainOneUnit(const Vector& x, const Vector& y,
    ...
    for (; linear_it != linear_layers_.end() &&
           non_linear_it != non_linear_layers_.end();
         ++linear_it, ++non_linear_it) {
        temp = linear_it->forwardWithoutShrinking(temp);
    ...
    for (; linear_layer_it != linear_layers_.rend() &&
        ...
        SVD g = linear_layer_it->backwardCalcGradient(u, *non_linear_in_it);
        to_update.emplace_back(g);
    }
    return to_update;
}

void GivensNet::addGradients(std::vector<SVD>& a, const std::vector<SVD>& b) {

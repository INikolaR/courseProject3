...
//         x.emplace_back(s == "C");
//         if (i < 99 * 8) {
//             train_dataset.emplace_back(TrainUnit{std::move(x),
//             std::move(y)});
//         } else {
...
void test_echo() {
    ...
    Net net(Linear{GivensLayer(In(1), Out(1), {0.9, 0.1})},
            NonLinear::LeakyReLU());
    Optimizer optimizer = Constant(0.02);
    for (int q = 0; q < 100; ++q) {
...
//                     Optimizer householder_opt;
//                     if (optim == 0) {
//                         givens_opt = Constant(givens_net.linearLayers(),
//                         step); matrix_opt =
//                         Constant(matrix_net.linearLayers(), step);
//                         householder_opt =
//                             Constant(householder_net.linearLayers(), step);
//                     } else if (optim == 1) {
//                         givens_opt =
...
//                         householder_opt =
//                             Momentum(householder_net.linearLayers(), step,
//                             0.5);
//                     } else {
...
//                             getClassificationReport(givens_metrics,
//                             givens_net,
...
//                             getClassificationReport(matrix_metrics,
//                             matrix_net,
//                                                     train, loss, test, loss);
...
//     std::vector<LossFunction> losses = {LossFunction::Euclid()};
//     std::vector<Vector> architectures = {{9, 30, 2}, {9, 1000, 2}, {9, 10,
//     2}}; size_t batch_size = 8; double step = 0.0001; size_t n_of_epochs = 5;
//     for (Vector architecture : architectures) {
//         for (LossFunction loss : losses) {
